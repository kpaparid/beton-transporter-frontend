...
export const MonthlyWorkHours = (props) => {
  ...
  return (
    <div className="table-wrapper table-responsive shadow-sm flex-fill ">
      <Card
        border="light"
        className="table-wrapper table-responsive shadow-sm flex-fill"
      >
        ...
        <Card.Body>
          <Table hover className="user-table align-items-center">
            ...
            <tbody>
              {workHours.map((t) => (
                <TableRow key={`monthlyWorkHours-${t.id}`} {...t} />
              ))}
            </tbody>
          </Table>
        ...
        <Card.Body className="p-0 p-sm-1 table-responsive-sm table-condensed">
          <Table hover className="table table-sm">
            <thead className="thead-light">
              <tr className="text-center">
                {headers.map((h, id) => (
                  <th key={`header-${title}-${id}`} className="border-bottom">
                    <span className="fw-bolder f-lg">{h}</span>
                  </th>
                ))}
              </tr>
            </thead>
