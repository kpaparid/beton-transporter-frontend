...
export default (props) => {
    ...
    const { message, setMessage, receivers, setReceivers, clearData } = props.data;
    ...
    const handleAddReceiver = (event) => {
        ...
        if (value !== defaultValue) setReceivers(r => [...new Set([...receivers, value])])
    }
    ...
    const ReceiversBtn = () => {
        return (
            <div className="d-flex mt-1 flex-wrap">
                {receivers.map((r, index) => <CustomBtn className="m-1" key={index} name={r}></CustomBtn>)}
            </div>
        );
    };
    const CustomBtn = (props) => {
        ...
        return (
            <ButtonGroup className={`d-flex alignt-items-center ${className}`}>
                <Button>{name}</Button>
                <Button variant="danger" size="sm" value={name} onClick={handleRemoveReceiver}>x</Button>
            </ButtonGroup>
        );
    };
        ...
        <>
            <Card border="light" className="shadow-sm flex-fill my-2 d-flex">
                ...
                <div className="px-4 d-flex flex-fill flex-wrap flex-lg-nowrap align-items-center justify-content-center w-100">
                    <h3 className="m-4 ms-6  text-center container-fluid">Neue Nachricht</h3>
                    ...
                    <Form>
                        <Form.Group className="mb-3">
                            ...
                            <Form.Select value={defaultValue} onChange={handleAddReceiver}>
                                ...
                                {teamMembers.map((t, index) => <option key={index}>{t.name}</option>)}
                            </Form.Select>
                        ...
                        <Form.Group className="mb-3">
                            ...
                            <Form.Control as="textarea" name="title" rows="1" value={message.title} onChange={handleTextChange}></Form.Control>
                        </Form.Group>
                        <Form.Group className="mb-3">
                            <Form.Label>Nachricht</Form.Label>
                            <Form.Control as="textarea" name="mail" rows="10" value={message.mail} onChange={handleTextChange} />
                        </Form.Group>
                        <Form.Control type="file" className="mb-3" />
                        <Button variant="primary" className="my-3" onClick={handleSend}>Send</Button>
                        <MyModal message={message} onClose={handleClose} show={showModalDefault}></MyModal>
