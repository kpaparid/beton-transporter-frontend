...
import com.bt.btbackend.security.models.Credentials.CredentialType;
import com.bt.btbackend.security.models.SecurityProperties;
import com.bt.btbackend.security.models.User;
import com.bt.btbackend.security.roles.RoleConstants;
...
public class SecurityFilter extends OncePerRequestFilter {
	...
	private void authorize(HttpServletRequest request) {
		...
		if (user != null) {
			// Handle Super Role
			List<String> c = securityProps.getSuperAdmins();
			...
			// Handle Other roles
			decodedToken.getClaims().forEach((k, v) -> {
				authorities.add(new SimpleGrantedAuthority(k));
			});

//			try {
//				firebaseAuth.getUser(decodedToken.getUid()).getCustomClaims().forEach((k, v) -> {
//					authorities.add(new SimpleGrantedAuthority(k));
//				});
//			} catch (FirebaseAuthException e) {
//				e.printStackTrace();
